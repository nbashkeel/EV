...
breast.essential <- as.data.frame(Essentiality(breast))
  breast.essential$Tissue <- "Breast"
cere.essential <- as.data.frame(Essentiality(cerebellum))
  cere.essential$Tissue <- "Cerebellum"
frontal.essential <- as.data.frame(Essentiality(frontal))
  frontal.essential$Tissue <- "Frontal Cortex"

ggDF <- rbind(as.data.frame(breast.essential),
              as.data.frame(cere.essential),
              as.data.frame(frontal.essential))
ggDF %>% mutate("Probe Set" = rownames(ggDF)) %>%
  select("Tissue", "Probe Set", 1:4)
